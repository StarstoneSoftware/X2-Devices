...
/*! \page x2howto How to Write a TheSkyX Driver
	...
	<h3>TheSky64 - TheSky Windows 64 Bit X2 Drivers</h3>
	In early 2021, TheSky on <i>Windows</i> became available as <i>64 bit</i>. The following section applies to Windows only, when creating a 64 bit x2 driver.  All other supported operating systems are/were already 64 bit, and do not have the consideration
	of creating/distributing 32 and or 64 bit drivers.
	A 64 bit x2 driver is identical to the above procedure, except for the following differences:
	-# When compiling, build the binary for 64 bit.
	-# The hardware list file must be differentiated with a "64". Following the above example, "cameralist64 My Company.txt" (note the "64").
	-# The binary must be placed int the "TheSkyX/Resources/Common/PlugIns64/CameraPlugIns" folder (note the "64").
	
	In short, a 64 bit x2 driver is built for 64 bit, has a 64 bit specific hardwarelist.txt file, and is placed in a 64 bit specific folder.  This way 32 bit drivers and 64 drivers are completely independent of one another.
	This convention also means a 64 bit x2 driver binary can have the same name as a 32 bit build, but for clarity, we encourge 64 bit drivers, especially those that distribute both 32 and 64, to have a different name for 
	64 bit (append "64" to the binary name before the extenstion suffix, like "x2camera64.dll").  A standard TheSky installation on Windows has several good examples of 64 bit x2 drivers that can be examined and followed.
*/
...
/*!	\page x2changelog Change Log
	...
	- 1.22 - Added power control support, specifically PowerControlDriverInterface.
	- 1.23 - Documented how to create/distribute a 64 bit, x2 driver for Windows within \ref x2howto.
	- 1.24 - Documented the WeatherStationDataInterface. Documented the DomeHasHighlyRelaibleOpenCloseSensors interface.
